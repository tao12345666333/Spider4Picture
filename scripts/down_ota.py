...
class TitleName(SGMLParser):
    ...
    def start_a(self, attrs):
        global fname, durl
        self.urls = []
        href = [v for k, v in attrs if k ==
                'href' and v.split('.')[-1] == 'jpg']

        if href:
            self.urls.extend(href)
            urlretrieve(href[0], '/home/tao/spider/%s' %
                        href[0].split('/')[-1])
            durl = href[0]
...
for sid in range(134000, 135493):
    ...
    try:
        ...
        for item in titlename.name:
            print item,
            print url_from_sid
            pic.insert({"title": item, "url": url_from_sid,
                        "name": fname, "durl": durl, "atime": datetime.now()})
